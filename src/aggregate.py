...
INPUT_DIR = pathlib.Path("datasets/raw/complementarios")
OUTPUT_DIR = pathlib.Path("datasets/complementarios/")
...
def process_month(month_dir: pathlib.Path):
    ...
    out_path = OUTPUT_DIR / f"complementarios_tripdata_2025-{month}.csv.gz"
...
def main():
    ...
    # Each subfolder = one month
        ...
        if not month_dirs:
            print("⚠️ No month subfolders found in data/raw/")
            return
